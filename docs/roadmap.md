...
- **v1 — Agent Layer (Phases 13–15) — COMPLETE (2026-07-21).** Phases 13, 14, 15 shipped (2026-07-20 → 07-21); the project is now in **demand-gated maintenance**. **Phase 13.5 (local MCP wrapper) is descoped from v1 to an optional post-v1 add-on** — [issue #35](https://github.com/xy9iao/ai-stock-analyst/issues/35), build on demand. v1 measured: 79% agent-path prompt-cache hit; ~20% net long-chat compression saving; regression gate at 0.983 + citation + poisoned-chunk cases.
- Anything not listed in the v1 plan is **out of scope by default**; deferred ideas stay as GitHub issues.

## v1 — Agent Layer: Research Agent, RAG, Compression & Injection Defense (Phases 13–15)
...
### Phase 13.5 — MCP Wrapper (local stdio only) — POST-V1 ADD-ON

Descoped from v1 to an optional, unscheduled add-on (2026-07-21). Full spec and acceptance criteria live in **[issue #35](https://github.com/xy9iao/ai-stock-analyst/issues/35)** — build on demand only. Core constraint: local stdio only, never a hosted endpoint (it would bypass session caps and the master switch).
...
### v1 Close-Out — DONE (2026-07-21)

- ✅ README: architecture diagram shows the agent / RAG / compression paths; caching + compression numbers present; status → v1 complete, maintenance mode.
- ✅ decisions.md: hand-written loop vs LangGraph (010), routing + non-agentic chat (011), hybrid retrieval (012), injection threat model + depth (013).
- ✅ CLAUDE.md: demand-gated maintenance rule + the regression-set merge gate (binding).
- ✅ CHANGELOG closes v1; project in demand-gated maintenance. MCP wrapper → optional add-on, [issue #35](https://github.com/xy9iao/ai-stock-analyst/issues/35).
