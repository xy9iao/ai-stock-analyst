...
def send_message(request: ChatMessageRequest, db: Session = Depends(get_db)) -> ChatReply:
    session_id, reply = service.send_message(db, request)
    return ChatReply(session_id=session_id, reply=reply)


@router.get("/sessions")
def list_sessions(db: Session = Depends(get_db)) -> list[ChatSessionRead]:
    return repository.list_sessions(db)


@router.get("/sessions/{session_id}/messages")
def list_messages(session_id: int, db: Session = Depends(get_db)) -> list[ChatMessageRead]:
    if repository.get_session(db, session_id) is None:
        raise AppError(
