...
def create_report(request: ReportRequest, db: Session = Depends(get_db)) -> ReportRead:
    return report_generator.generate_report(db, request)


@router.get("")
def list_reports(db: Session = Depends(get_db)) -> list[ReportRead]:
    return repository.list_reports(db)
