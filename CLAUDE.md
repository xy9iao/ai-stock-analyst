...
## Core/Periphery Split (v1+, binding)

- Every v1 phase plan splits its work into two declared tiers: **Core** and **Periphery**, labeled per item at phase kickoff.
- **Core code is owner-written.** CC implements periphery (migrations, routers/wiring, frontend rendering, glue, test scaffolding, config) under normal review.
- **Regression gate:** from Phase 13 on, any change to prompts, models, or retrieval parameters must run the `eval/` regression set before merge; a score drop blocks the merge.
- Process details: `teaching-protocol.md` (gitignored).
